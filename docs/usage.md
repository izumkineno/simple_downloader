...
```toml
[dependencies]
simple_downloader = { version = "0.1", features = ["resume"] }
```
...
```toml
[dependencies]
simple_downloader = { version = "0.1", features = ["progress"] }
```
...
```toml
[dependencies]
simple_downloader = { version = "0.1", features = ["multi-source", "progress"] }
```
...
```toml
[dependencies]
simple_downloader = { version = "0.1", features = ["proxy", "progress"] }
```
        ...
        .client_builder(|| {
            ClientBuilder::new()
                ...
                .default_headers({
                    ...
                    h.insert("User-Agent", "simple_downloader/0.1".parse().unwrap());
                    h
                })
                // 危险：仅测试环境
