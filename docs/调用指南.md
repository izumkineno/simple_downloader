...
本文是 `simple_downloader` 的**调用侧权威文档**，覆盖所有公开 API 的最小可运行调用形态、Feature 选型、错误处理与完整示例索引。设计细节见 [`架构说明.md`](./架构说明.md)，配置项详解见 [`配置参考.md`](./配置参考.md)，错误全表见 [`错误说明.md`](./错误说明.md)。

> 基线：`0.7.1`，`edition 2024`，`rust 1.85`。`default = []`，未启用的 feature 对应 API 不存在。
...
| `speed_limit(bps)` | `rate-limit` | 全局限速 `bytes/s`，`0/>u32::MAX` 均 `InvalidArgument`，`burst` 未显式指定时默认 `64KiB` | `.speed_limit(1_048_576)` |
...
不启用任何 feature 即可用，自动处理 `HEAD -> Range: bytes=0-0 -> Content-Length` 回退（含 `416 bytes */total` 解析，见 `src/util.rs:get_file_info_with_headers`）。`HEAD` 与 `Range` 均失败且无大小时回退单流流式（`total_size=0`），`Range` 请求强制 `identity`。
...
降级行为：服务器不支持 `Range` 或文件 `<1 MiB` 时自动单线程（见 §11）。收尾以落盘长度为金标准，不一致返回 `Io(UnexpectedEof)` 并保留 sidecar。

---

## 5. 场景二：断点续传（`resume`）

启用 `resume` 后，`Downloader` 自动在 `output_path` 同级维护 `output_path.download.bitcode` sidecar（`src/resume.rs:RESUME_EXTENSION`），基于固定 `64KiB` segment 哈希校验恢复，非简单追文件长度。

```toml
[dependencies]
simple_downloader = { version = "0.7", features = ["resume"] }
```
...
恢复语义（`src/resume.rs` / `src/downloader.rs`）：

- 仅复用哈希校验通过的 segment，其余重建为待下载区间；`METADATA_VERSION = 2`，`version/file_size/segment_size` 任一不一致即删 sidecar 重建（`v1` 脏数据自愈）。
- `sidecar` 存在但目标文件缺失 → `Err(ResumeTargetMissing)` fail-stop，不静默重下。
- 已验证 segment 被外部篡改 → 该 segment 失效重下。
- 单源/多源统一走 `ResumePlan`；成功后自动删 sidecar（3 次重试），失败/中断残留下次复用。
- 预留字段：`ResumeMetadata { etag, last_modified, effective_url }` 已入库、`adaptive_segment_size(≤100M→64K/≤1G→256K/>1G→1M)` 已导出，但**当前均未接线**——实际仍为固定 `64KiB`，ETag 变更暂不触发失效重建。

工具 API（`resume` feature 导出）：

```rust
use simple_downloader::{metadata_path_for, hash_bytes, ResumeMetadata, DEFAULT_SEGMENT_SIZE, adaptive_segment_size};
...
let seg = adaptive_segment_size(file_size); // 仅预估用，ResumePlan 暂未采用
```
...
```toml
[dependencies]
simple_downloader = { version = "0.7", features = ["progress"] }
```

`run(handler)` 将 `total_size: u64` 与 `broadcast::Receiver<DownloadInfo>` 交给调用方，调用方在独立 task 中消费事件。`DownloadInfo` 变体见 `src/types.rs`。
    ...
    Downloader::builder("https://proof.ovh.net/files/100Mio.dat", "100Mio.dat")
        ...
        .run(|total_size, mut info_rx| async move {
            ...
            while let Ok(info) = info_rx.recv().await {
                match info {
                    DownloadInfo::MonitorUpdate { total_downloaded, total_speed, chunk_details, eta_secs, .. } => {
                        // 常用快捷方法
                        println!(
                            "进度 {:.1}% 速度 {:.2} MB/s {}/{} ETA {:?}",
                    ...
                    _ => {} // #[non_exhaustive] 新增变体 minor 兼容
                }
            }
        })
        .await?;
...
`DownloadInfo` 快捷方法（**稳定契约**，非 `MonitorUpdate` 固定返回 `0/false/None`，见 `src/types.rs:DownloadInfo` 顶层文档）：

| 方法 | 适用变体 | 说明 | 兼容 |
|---|---|---|---|
| `progress_percent() -> f64` | `MonitorUpdate` | `0.0~100.0`，`total_size==0` 时 `0.0` | stable |
| `speed_mbps() -> f64` | `MonitorUpdate` | `total_speed / 1MiB`（EMA 求和，限速后观测值） | stable |
| `downloaded_bytes() -> u64` | `MonitorUpdate` | `total_downloaded`（已 `min(total)` 钳制，仅展示） | stable |
| `total_bytes() -> u64` | `MonitorUpdate` | `total_size`，`0` 表示未知或 0 字节文件 | stable |
| `is_complete() -> bool` | `MonitorUpdate` | `total_downloaded >= total_size`；`0/0 true，0/N false`（流式以 `DownloadComplete` 为准，**仅展示用**） | stable |
| `eta_secs() -> Option<u64>` | `MonitorUpdate` | `remaining/total_speed`，进度 `<5%` 或速度为 0 时 `None` | stable (0.7+) |
| `pieces() -> Option<&[bool]>` | `MonitorUpdate` | 段完成位图，**当前恒为空**，待 resume 位图接入 | stable (0.7+) |

**UI 稳定契约（0.6.2+，SemVer Minor 兼容）**：
- `#[non_exhaustive]`：新增变体为 minor，`match` 必须含 `_` 分支。
- `MonitorUpdate` 新增字段为 minor：旧代码用 `..` 忽略即可（`eta_secs/pieces` 即如此加入）。
...
- **终局判定**：`is_complete()` 仅展示，真正的完成以 `DownloadComplete` + 落盘 `stat` 校验为准；进度条不要在 `is_complete()` 为真时直接 `100%` 收尾，应等待 `run()` 返回 `Ok`（0.7.1 收口语义）。

> 回调内避免阻塞/耗时 IO，必要时通过 `mpsc` 转发到其他任务（见 [`最佳实践.md`](./最佳实践.md) 进度回调节）。
---
...
```toml
[dependencies]
simple_downloader = { version = "0.7", features = ["multi-source", "progress"] }
```
...
行为（`src/lane.rs` / `src/downloader.rs`）：
...
- 按 `64KiB probe_speed` 实测排序建调度器，lane 连续失败 `>=3` 次（`BLACKLIST_THRESHOLD`）进黑名单 `30s` 后自动解封。
- 断点续传与多源正交，`resume` 启用时同样基于 segment 哈希恢复，与 `new_multi` 共享 `ResumePlan` 路径。
- 无 `weight/priority` 概念，旧文权重 API 已删除。

`SourceConfig` API：`new(url)`、`with_id(id)`、`with_proxies(vec)`（需 `proxy` feature）、`with_speed_limit/with_burst`（需 `rate-limit`）。

`MultiSourceConfig` API：`new(output, workers, update_interval)`、`with_sources(vec)`、`with_lane_model()`、`with_max_chunks_per_lane(n)`、`with_max_chunks_per_source(Option<usize>)`、`with_global_speed_limit/with_global_burst`（需 `rate-limit`）。
...
```toml
[dependencies]
simple_downloader = { version = "0.7", features = ["proxy", "progress"] }
```
...
`ProxyConfig` 构造（`src/lane.rs`）：`ProxyConfig::http(url)`、`ProxyConfig::https(url)`、`ProxyConfig::socks5(url)`，亦可通过 `client_builder` 直接注入 `reqwest::Proxy`（见 §9）。单 lane `Client::build` 失败仅跳过该 lane，不拖垮全源。
    ...
    Downloader::builder("https://example.com/file.bin", "output.bin")
        ...
        .client_builder(|| {
            ClientBuilder::new()
                ...
                // 自定义 Header（UA 缺失时库自动注入 simple_downloader/x.y.z）
                .default_headers({
                    let mut h = reqwest::header::HeaderMap::new();
                    h.insert("X-Custom", "demo".parse().unwrap());
                    h
                })
                // 危险：仅测试环境
...
> `client_builder` 为 `Fn() -> ClientBuilder`，每次探测/重试会重新 `build()`，确保多源场景下每个 lane 独立 `Client`；`with_client_builder` 时保留用户 `pool/tcp_keepalive` 设置，不被默认值覆盖。

---

## 10. 错误处理模板

完整错误变体与重试建议见 [`错误说明.md`](./错误说明.md)。`src/types.rs` 定义：

| 变体 | 场景 | 可重试 |
|---|---|---|
| `Request(reqwest::Error)` | 网络/DNS/超时/4xx/5xx/TLS | 部分（超时/连接/5xx；`403/404/416` 为 Permanent 不重试） |
| `Io(io::Error)` | 磁盘满/权限/占用/落盘金标准 `UnexpectedEof` | 否 |
| `Join(JoinError)` | task panic/取消 | 是 |
| `MissingContentLength` | 无 `Content-Length` 且流式回退亦失败 | 否（0.3.1+ 通常已自动回退单流流式） |
| `NoAvailableSources` | 多源全不可用 | 是（增源后） |
| `ResumeTargetMissing(PathBuf)` | sidecar 存在但文件缺失 | 否（补文件/删 sidecar/`resume(false)`） |
| `ResumeMetadata(String)` | 元数据损坏/版本不兼容 | 否（删 sidecar 重建，`version/size/segment` 不一致自愈） |
| `PermanentFailure(String)` | 跨周期 `>30` 熔断 / `Permanent` 分类 / 本地写入击穿 | 否（查网络/源/磁盘） |
| `InvalidArgument(String)` | `speed_limit/burst` 为 0/超限/`burst` 无 `speed_limit` | 否（修正参数） |
...
- 运行时 `DownloadState`/`ConcurrencyManager` 在 `Probing -> Stable` 两阶段动态探测：`Probing` 需正向吞吐增益才扩容，`Stable` 仅在吞吐相对历史基线显著回落且剩余时间仍值得（`MIN_REMAINING_TIME_FOR_SPLIT 3s`）时才切最慢可分片块；补位亦按 `remaining 可分片量` 而非原始尺寸选目标（`MIN_SPLITTABLE_REMAINING 256KiB`，`MIN_CHUNK_SIZE 10KiB`）。详见 `架构说明.md:动态分片` 与 `tests/concurrency.rs`。
- `rate-limit` 启用时 `DownloadMonitor::is_rate_limited=true` 冻结 `decide_and_act`，`drain_pending` 容量补位除外，避免限速被误判为带宽不足而过度分裂。
- 完成判定：`completed_bytes >= total` 落账权威，`total_downloaded()` 已 `min(total)` 钳制，`is_complete()` 仅展示。

建议：小文件 `<100 MiB` 用 `4-8` workers，大文件 `>1 GiB` 用 `16-32`，多源可 `32` 但受 `max_chunks_per_lane/source` 约束。
...
```toml
[dependencies]
simple_downloader = { version = "0.7", features = ["rate-limit", "progress"] }
```
...
#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // 单源全局限速 1 MiB/s，不指定 burst 即默认 64KiB
    Downloader::builder("https://example.com/file.bin", "output.bin")
        .workers(16)
...
- `1 token = 1 byte`，`Quota::per_second(limit).allow_burst(burst)`，`burst None` 默认 `64KiB` 硬限，`chunk` 内 `32-64KiB` 批量 `tokio::join!(per.acquire, global.acquire)`。
- `is_rate_limited` 时 `Monitor` 跳过 `decide_and_act`，`drain_pending` 仍补位（冻结例外）；`RuntimeConfig` 支持运行时全局限速 `reconfigure/disable`，`per_source` 热更待补。
- 小限速防死锁：`small_limit_not_deadlock` 覆盖（显式小 burst 仍可推进）。
...
```toml
[dependencies]
simple_downloader = { version = "0.7", features = ["queue"] }
```
...
- 取消删除：`pending_deletes` 200ms 周期 `flush_pending_deletes`，`NotFound` 视成功，`PermissionDenied/未知Io` `warn+重试`。
- `TaskState: Queued/Paused/Active/Completed/Failed/Cancelled/Removed`，`QueueError` 仅队列层错误，下载层错误在 `TaskSnapshot`。
- 终局可靠：`ChunkFailed/Complete/Bisected` 经 `reliable mpsc` 兜底防 `Lagged` 丢失。
...
| 速度防护演示 | `examples/speed_guard_demo.rs` | — | tiny elapsed/巨大瞬时/零增量三档防护 |
| 高并发回归 | `examples/regression_high_concurrency.rs` | — | state 模拟 600M/8 完成 |
...
- [ ] `Cargo.toml` feature 按需开启，`default-features = false` 组合是否满足需求（默认无任何可选能力）？
- [ ] `workers` / `update_interval` 是否符合文件大小与 UI 刷新需求？
- [ ] 需要断点续传时 `resume` 是否启用，sidecar 清理策略是否明确（成功自动删 `*.download.bitcode`，中断残留下次 hash 复用；`version/size/segment` 不一致自愈重建）？
- [ ] 需要进度时 `progress` + `run(handler)` 回调内是否无阻塞操作（`mpsc` 转发）且 `match` 含 `_` 分支（`#[non_exhaustive]`）？`is_complete()` 是否仅展示，终局等待 `run()` 返回 `Ok`？
- [ ] 多源场景源是否同文件同大小且支持 `Range`，`LaneModel` 是否匹配代理维度（`PerSource`/`PerSourceProxy`）？
- [ ] 限速场景 `speed_limit/burst` 是否校验且 `global` 为硬上限（`per_source` 之和受 `global` 约束，`join max`）？未显式 `burst` 即 `64KiB` 是否符合预期？
- [ ] 队列场景 `with_max_concurrent` 是否 `1..64` 且 `enqueue` 同路径是否接受 `a(N).ext` 重命名（跨进程需外部锁）？
- [ ] 自定义 `ClientBuilder` 超时/连接池（`pool 32/90s/60s`）是否按文件大小配置？
- [ ] 错误分支是否区分可重试/不可重试（`Request::is_timeout` / `ResumeTargetMissing` / `InvalidArgument` / `PermanentFailure` 等）？
