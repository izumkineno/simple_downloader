...
本文档介绍如何在项目中安装和使用 simple_downloader 库。以 `Cargo.toml` 与 [`调用指南.md`](./调用指南.md) 为准。

> 基线 `0.7.1`，`edition 2024`，`rust-version 1.85`。
...
- 本库 `reqwest` 默认走 `rustls`，**无需** 系统 `openssl`/`libssl-dev`。旧文提及的 `libssl-dev` 仅适用于 `native-tls` 场景，本库无需。
...
### Q: 代理不生效 / 断点续传不可用
检查 `Cargo.toml` 是否按需启用了 `proxy`/`resume`，并重建 `cargo clean && cargo build --all-features`。`default = []` 下裸依赖无代理、无续传、无进度。
