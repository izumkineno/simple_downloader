...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if is_multi {
        #[cfg(all(feature = "rate-limit", feature = "multi-source"))]
        {
            ...
            let out =
                std::env::var("TEST_OUT").unwrap_or_else(|_| "10Mb_multi_rate.dat".to_string());
            println!(
                "多源限速: s1 300KiB/s burst 64KiB + s2 300KiB/s burst 32KiB, 全局 512KiB/s burst 64KiB -> {}",
                out
            );
            let cfg = MultiSourceConfig::new(out.clone(), 8, 0.5)
                .with_sources(vec![
                    SourceConfig::new(url.clone())
                        .with_id("s1")
                        .with_speed_limit(300 * 1024)
                        .with_burst(64 * 1024),
                    SourceConfig::new(url)
                        .with_id("s2")
                        .with_speed_limit(300 * 1024)
                        .with_burst(32 * 1024),
                ])
    ...
    {
        eprintln!(
            "请启用 rate-limit feature: cargo run --features rate-limit,progress --example with_rate_limit"
        );
